...
class Agent:
    def __init__(self, buffer, model_buffer, best_buffer, environment):
    ...
    def advance_state_and_log(self, model, epsilon, device="cpu"):
        ...
        if done:
            ...
            if self.no_clear:
                self.best_time = t_reward
                self.best_buffer = copy.deepcopy(self.model_buffer)
            else:
                if t_reward >= self.best_time:
                    self.best_time = t_reward
                    self.best_buffer = copy.deepcopy(self.model_buffer)

            self.model_buffer.wipe()
...
replay_buffer = ReplayBuffer(buffer_size) #stores the best trajectory and trains on it
trial_buffer = ReplayBuffer(model_size) #tries out other trajectories
best_buffer = ReplayBuffer(model_size)
agent = Agent(replay_buffer, trial_buffer, best_buffer, env)
...
episode_num = 0
elapsed = 0
best_5_avg = -5000 #value is pretty much impossible to achieve even with random policy
...
for step in range(max_steps):
    ...
    if replay_buffer.size() > 2*batch_size:
        ...
        if episode_num > 0:
            ...
            if chance < 1:
                cur_state_arr, action_arr, reward_arr, done_arr, next_state_arr = agent.check_best().sampleBuf(batch_size)
...
with open('dqn2_loss_array.txt', 'w') as f:
    f.write(str(loss_save) + '\n')
    f.close()

with open('dqn2_reward_array.txt', 'w') as f:
    f.write(str(return_save) + '\n')
    f.close()

torch.save(behavior_model.state_dict(), './dqn2_model_excitebike.pth')
torch.save(best_state_dict, './dqn2_best_model_excitebike.pth')
